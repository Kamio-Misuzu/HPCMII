...
logging.basicConfig(level=logging.INFO,format='%(asctime)s %(message)s', filename='./logs/nets.txt')
...
def main():
    init_env('0')
    model = Model_3D(num_cls=config['number_classes'])
    ...
    lr_schedule = LR_Scheduler(base_lr, config['epochs'], warmup=0, mode='warmuppoly')
    train_params = [{'params': model.parameters(), 'lr': base_lr, 'weight_decay': weight_decay}]
    optimizer = torch.optim.AdamW(train_params,  betas=(0.9, 0.999), eps=1e-08, amsgrad=True)

    if config['path_to_resume'] is not None and config['path_to_resume'] != 'None':
        checkpoint = torch.load(config['path_to_resume'], weights_only=False)
    ...
    for epoch in range(epoch_init, n_epochs):
        ...
        for batch_id, (batch_x, batch_y, patient_id) in enumerate(loader):
            ...
            optimizer.zero_grad()
            with autocast():
                fuse_pred, prm_preds = model(batch_x, mask)
                ###Loss compute
