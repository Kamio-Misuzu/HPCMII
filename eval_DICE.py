...
logging.basicConfig(level=logging.INFO,format='%(asctime)s %(message)s', filename='./logs/eval_DICE_nets.txt')
...
def main():
    ...
    if config['path_to_resume'] is not None:
        ...
        for k, v in pretrained_dict.items():
            if k.startswith('module.') and not any(key.startswith('module.') for key in model_dict.keys()):
                new_key = k[7:]
                if new_key in model_dict:
                    new_pretrained_dict[new_key] = v
            elif not k.startswith('module.') and any(key.startswith('module.') for key in model_dict.keys()):
                new_key = 'module.' + k
                if new_key in model_dict:
                    new_pretrained_dict[new_key] = v
            else:
