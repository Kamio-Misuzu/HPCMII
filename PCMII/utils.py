...
class LR_Scheduler(object):
    ...
    def __call__(self, optimizer, epoch):
        if self.mode == 'poly':
            now_lr = round(
                self.lr * float(np.power(1 - np.float32(epoch)/np.float32(self.num_epochs), 0.9)),
                8
            )
            ...
            if epoch < 50:
                now_lr = round(
                    self.lr * float(((np.float32(epoch+1.0))/(np.float32(50.0)))),
                    8
                )
            else:
                now_lr = round(
                    self.lr * float(
                        np.power(
                            1 - (np.float32(epoch) - np.float32(50.0)) /
                            (np.float32(self.num_epochs+1)-np.float32(50.0)),
                            0.9
                        )
                    ),
                    8
                )
        self._adjust_learning_rate(optimizer, float(now_lr))
        return now_lr
...
def test_softmax(
    ...
    if dataname in ['/path/to/BRATS2024', '/path/to/BraTS/BRATS2020', '/path/to/BraTS/BRATS2018']:
        num_cls = 4
