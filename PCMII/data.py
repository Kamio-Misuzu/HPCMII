...
class Brats2024(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for mode in modes:
            ...
            brats_mode = mode_mapping.get(mode, mode)
            volume_path = os.path.join(patient_dir, patient_id + "-" + brats_mode + '.nii.gz')
            if not os.path.exists(volume_path):
                volume_path = os.path.join(patient_dir, patient_id + "-" + brats_mode + '.nii')
...
def split_dataset(data_root, test_p):
    ...
    Args:
        ...
        test_p: test set ratio
    """
    patients_dir = glob.glob(os.path.join(data_root, "BraTS-GLI-*"))
    patients_list = sorted(patients_dir)
    
    N = int(len(patients_list) * test_p)
    train_patients_list = patients_list[N:]
    val_patients_list = patients_list[:N]

    return train_patients_list, val_patients_list
    
def make_data_loaders(config):
    data_root = config.get('path_to_data', None)
    if data_root is None:
        raise ValueError("path_to_data must be specified in config")
    
    train_list, val_list = split_dataset(data_root, float(config['test_p']))
    ...
    train_ds = Brats2024(train_list, crop_size=crop_size, modes=config['modalities'], train=True)
    val_ds = Brats2024(val_list, crop_size=crop_size, modes=config['modalities'], train=False)
